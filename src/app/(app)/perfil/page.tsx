...
import {
  ...
  Pencil, Check, X, Users, Wallet, LogOut, ShieldCheck, Mail
} from 'lucide-react';
...
export default function PerfilPage() {
  ...
  const { perfil, loading, refetchPerfil, aplicarPatchLocal } = usePerfil();

  const [ddrVer] = useState(DDR_FALLBACK);
  ...
  useEffect(() => {
    if (!perfil) return;
    setBio(perfil.bio || '');
    setLane(perfil.lanePrimaria || 'Top');
    setLane2(perfil.laneSecundaria || 'Support');
    setInstagram(perfil.instagram || '');
    setTwitch(perfil.twitch || '');
    setYoutube(perfil.youtube || '');
    setDiscord(perfil.discord || '');
    if (perfil.pix) {
      setChavePix(perfil.pix.chave || '');
      setTipoChavePix(perfil.pix.tipo || '');
      setNomePix(perfil.pix.nome || '');
    }
  }, [perfil]);
  ...
  const handleLane = async (id: string, isSecondary = false) => {
    if (isSecondary) {
      setLane2(id);
      setLane2Open(false);
      aplicarPatchLocal({ laneSecundaria: id });
    } else {
      setLane(id);
      setLaneOpen(false);
      aplicarPatchLocal({ lanePrimaria: id });
    }
  };
  ...
  const handleSalvarBio = () => {
    ...
    aplicarPatchLocal({ bio: bioTemp });
  };

  const handleSalvarPix = (tipo: string, chave: string, nome: string) => {
    setChavePix(chave);
    setTipoChavePix(tipo);
    setNomePix(nome);
    setModalPixAberto(false);
    aplicarPatchLocal({ pix: { tipo, chave, nome } });
  };

  const handleSair = () => { router.push('/lobby'); };
  ...
  const displayName = perfil?.displayName || 'Jogador';
  const avatarUrl = perfil?.avatarUrl;
  const isVip = perfil?.isVip || false;
  const eloPrincipal = perfil?.eloPrincipal || 'Unranked';

  // Deriva a Tag da Riot da primeira conta de jogo verificada ou riotId
  const riotAccount = perfil?.gameAccounts?.find(a => a.gameId === 'lol') || perfil?.gameAccounts?.[0];
  const riotHandle = riotAccount ? `#${riotAccount.handle}` : perfil?.riotId ? `#${perfil.riotId}` : '';

  const equipe = perfil?.meuTime;
  const eloSolo = perfil?.eloCache?.soloQ;
  const eloFlex = perfil?.eloCache?.flexQ;
  ...
  return (
    <div className="relative min-h-screen w-full text-white font-sans overflow-x-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto p-4 md:p-8 space-y-8 pb-32">
                ...
                <div className="text-center sm:text-left flex-1">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-2">
                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-headline font-black tracking-tight text-white uppercase break-all">
                      {displayName} <span className="text-white/40">{riotHandle}</span>
                    </h1>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
          ...
          {equipe ? (
            <motion.div
              whileTap={{ scale: 0.98 }}
              onClick={() => router.push(`/times/${equipe.id}`)}
              ...
              <div className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6">
                <div className="flex items-center gap-4 sm:gap-6 flex-1 min-w-0">
                  <div className="shrink-0">
                    <div
                      ...
                      style={{ border: `2px solid ${equipe.gradientFrom || '#FFB700'}`, background: 'black', boxShadow: `0 8px 24px -6px ${equipe.gradientFrom || '#FFB700'}60` }}
                    >
                      {equipe.logoUrl ? (
                        <img src={equipe.logoUrl} alt={equipe.nome} className="w-full h-full object-cover" />
                      ) : (
                        <span className="font-black text-base sm:text-lg tracking-widest" style={{ color: equipe.gradientFrom || '#FFB700' }}>{equipe.tag}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1.5 sm:mb-2">
                      ...
                      {equipe.tag && (
                        <span className="text-[10px] font-black px-2 py-0.5 rounded-md shrink-0" style={{ color: equipe.gradientFrom || '#FFB700', background: `${equipe.gradientFrom || '#FFB700'}18`, border: `1px solid ${equipe.gradientFrom || '#FFB700'}40` }}>
                          #{equipe.tag}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-4 sm:gap-6">
                      <div className="flex flex-col">
                        ...
                        <span className="font-black text-xs sm:text-sm" style={{ color: equipe.gradientFrom || '#FFB700' }}>{(equipe.pdl || 0).toLocaleString('pt-BR')}</span>
                      </div>
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="font-black text-xs sm:text-sm text-white">{equipe.wins || 0}/{(equipe.gamesPlayed || 0) - (equipe.wins || 0)}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between sm:justify-end gap-4 sm:gap-6 border-t border-white/5 pt-4 sm:border-t-0 sm:pt-0">
                  {(() => {
                    ...
                    const roleFile = equipe.membroRole ? ROLE_TO_LANE[equipe.membroRole.toUpperCase()] : null;
                    if (!roleFile) return null;
                    return (
                      <div className="shrink-0 flex items-center justify-center sm:pl-4 sm:pr-1">
                        <img
                          src={`/lanes/${roleFile}`}
                          alt={equipe.membroRole || ''}
                  ...
                  <div className="shrink-0 flex flex-col items-center justify-center sm:border-l sm:border-white/5 sm:pl-6 sm:pr-2 h-12 sm:h-16 gap-0.5">
                    <span className="text-sm sm:text-lg font-black tracking-tighter uppercase" style={{ color: equipe.gradientFrom || '#FFB700' }}>
                      RANK #{equipe.ranking || '—'}
                    </span>
                    <div className="flex items-center gap-1">
                      <span className="font-bold text-[9px] uppercase tracking-[0.15em]" style={{ color: equipe.gradientFrom || '#FFB700' }}>Ver Time</span>
                      <ChevronRight className="w-3 h-3" style={{ color: equipe.gradientFrom || '#FFB700' }} />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-white/5 rounded-2xl gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <span className="text-white/80 text-sm truncate">{perfil?.email || 'usuario@m7arena.pro'}</span>
                  </div>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={key} className="flex items-center gap-4 bg-white/5 p-3 rounded-2xl min-w-0">
                  {icon}
                  <input type="text" value={value} onChange={e => { let v = e.target.value; if (key === 'twitch') v = v.replace('https://www.twitch.tv/', '').replace('https://twitch.tv/', '').replace(/^@/, '').trim(); set(v); }} placeholder={placeholder} className="flex-1 bg-transparent px-0 py-1 text-white text-sm focus:outline-none placeholder:text-white/25 min-w-0" />
                </div>
              ))}
            </div>
          </motion.div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-4">
          <button onClick={() => router.push('/vincular')} className="flex-1 py-4 bg-white/5 border border-white/10 text-white rounded-2xl hover:bg-white/10 transition-all flex items-center justify-center gap-3 text-[10px] font-black uppercase"><LinkIcon className="w-4 h-4 text-primary" />Vincular Nova Conta</button>
      ...
      <ModalPix aberto={modalPixAberto} onFechar={() => setModalPixAberto(false)} onSalvar={handleSalvarPix} inicial={{ tipo: tipoChavePix, chave: chavePix, nome: nomePix }} />
    </div>
  );
}
