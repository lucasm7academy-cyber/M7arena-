...
import { Search, CheckCircle, AlertCircle, RefreshCw, ShieldCheck, Timer, Unlink, User, Gamepad2, Lock, Sparkles } from 'lucide-react';
...
export default function VincularPage() {
  ...
  useEffect(() => {
    const carregarContaVinculada = async () => {
      if (perfil) {
        if (perfil.gameAccounts && perfil.gameAccounts.length > 0) {
          const mainAcc = perfil.gameAccounts[0];
          setContaVinculada({
            riot_id: mainAcc.handle,
            puuid: mainAcc.externalId,
            summoner_id: mainAcc.id,
            level: 300,
            profile_icon_id: 29,
          ...
          setInvocador({
            riotId: mainAcc.handle,
            puuid: mainAcc.externalId,
            summonerId: mainAcc.id,
            nivel: 300,
            iconeId: 29,
            iconeUrl: getIconeUrl(29),
          });

          if (perfil.discord) {
            setDiscordVinculado(perfil.discord);
            setPasso('sucesso');
          } else {
            setPasso('validando');
          }
  ...
  const handleDesvincular = async () => {
    ...
    try {
      const res = await fetch('/api/vincular/desvincular', { method: 'POST' });
      if (res.ok) {
        setPopup({ type: 'success', message: '✅ Conta desvinculada com sucesso!' });
        await refetchPerfil();
        ...
        setPopup({ type: 'error', message: '❌ Erro ao desvincular conta. Tente novamente.' });
      }
    } catch {
      setPopup({ type: 'error', message: '❌ Erro ao desvincular conta. Tente novamente.' });
    } finally {
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      if (searchTerm.length >= 2) {
        try {
          const res = await fetch(`/api/riot/search?query=${encodeURIComponent(searchTerm)}`);
          if (res.ok) {
            const data = await res.json();
            setSugestoes(data.results || []);
          } else {
            setSugestoes([
              { riotId: `${searchTerm}#BR1`, iconId: 29, level: 342 },
              { riotId: `${searchTerm}#LAS`, iconId: 54, level: 198 },
            ]);
          }
        } catch {
          setSugestoes([
  ...
  const handleSelecionarJogador = async (riotId: string) => {
    ...
    try {
      const res = await fetch('/api/vincular/riot', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setJaVinculadaInfo({ riotId, email: 'outro usuário' });
          setPasso('ja_vinculada');
          setLoading(false);
          return;
        }
        setErro(errorData.message || 'Jogador não encontrado na Riot Games.');
        setLoading(false);
        return;
      }

      const data = await res.json();
      setContaVinculada(data.conta);
      setInvocador({
        riotId: data.conta.riot_id,
        puuid: data.conta.puuid,
        nivel: data.conta.level,
        iconeId: data.conta.profile_icon_id,
        iconeUrl: getIconeUrl(data.conta.profile_icon_id),
      });
      await refetchPerfil();
      setPasso('sucesso');
    } catch {
      // Mock fallback para ambiente de desenvolvimento local
      const contaData = {
        riot_id: riotId.includes('#') ? riotId : `${riotId}#BR1`,
        puuid: `puuid-${Date.now()}`,
        summoner_id: `sum-${Date.now()}`,
        level: 300,
        profile_icon_id: 29,
        created_at: new Date().toISOString(),
      };
      ...
      setInvocador({
        riotId: contaData.riot_id,
        puuid: contaData.puuid,
        nivel: contaData.level,
        iconeId: contaData.profile_icon_id,
        iconeUrl: getIconeUrl(29),
      });
      setPasso('sucesso');
    } finally {
  ...
  const handleVincularDiscord = async () => {
    ...
    try {
      ...
      const redirect = encodeURIComponent(`${window.location.origin}/api/auth/discord/callback`);
      window.location.href = `https://discord.com/oauth2/authorize?client_id=${clientId}&redirect_uri=${redirect}&response_type=code&scope=identify`;
    } catch {
            ...
            >
              ...
              <div className="relative p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-white/40 text-[10px] font-black uppercase tracking-[0.2em] font-headline">Conta Vinculada</h3>
                ...
                <div className="flex flex-col md:flex-row items-center gap-8 mb-8">
                  <div className="relative">
                    ...
                    <div className="relative">
                      <img 
                        src={invocador?.iconeUrl || getIconeUrl(contaVinculada.profile_icon_id || 29)} 
                  ...
                  <div className="text-center md:text-left">
                    <p className="text-3xl font-black text-white tracking-tight font-headline">{contaVinculada.riot_id}</p>
                    <div className="flex items-center justify-center md:justify-start gap-3 mt-2">
                      <span className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs font-bold text-white/60">
                        Nível {contaVinculada.level || 300}
                      </span>
                      <span className="text-[10px] text-[#FFB700] font-black uppercase tracking-widest flex items-center gap-1 font-headline">
                        <Sparkles className="w-3 h-3" /> Verificado
                      </span>
                    </div>
                    <p className="text-[10px] text-white/30 mt-3 uppercase tracking-widest">
                      Desde {new Date(contaVinculada.verified_at || contaVinculada.created_at || Date.now()).toLocaleDateString('pt-BR')}
                    </p>
                  </div>
                </div>
                ...
                {contaVinculada.elo && (
                  <div className="bg-white/[0.02] border border-white/10 rounded-xl p-5 mb-8 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[#FFB700] font-black text-xl italic uppercase tracking-tighter font-headline">{contaVinculada.elo}</span>
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <button
                    ...
                    className="py-4 rounded-xl bg-white/5 border border-white/10 text-white font-black text-sm uppercase hover:bg-white/10 transition-all flex items-center justify-center gap-2 font-headline cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="py-4 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 font-black text-sm uppercase hover:bg-red-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 font-headline cursor-pointer"
                  >
                    {desvinculando ? (
            ...
            >
              ...
              <div className="relative p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-white/40 text-[10px] font-black uppercase tracking-[0.2em] font-headline">Vincular Discord</h3>
                  <div className="flex-1 h-px bg-gradient-to-r from-[#5865F2]/30 to-transparent" />
                  {discordVinculado && (
                    <span className="text-[10px] text-[#5865F2] font-black uppercase tracking-widest flex items-center gap-1 font-headline">
                ...
                {discordVinculado ? (
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-6">
                    ...
                    <button
                      ...
                      className="shrink-0 px-8 py-4 rounded-xl font-black text-sm uppercase text-white transition-all hover:opacity-90 disabled:opacity-50 flex items-center gap-2 font-headline cursor-pointer"
            ...
            <div className="p-8">
                      ...
                      <div className="p-2">
                        {sugestoes.map((sug, index) => (
                          <motion.button
                            ...
                            className="w-full flex items-center gap-4 p-3 hover:bg-white/5 rounded-lg text-left transition-all group cursor-pointer"
                          >
                            <div className="relative">
                              <img 
              ...
              <div className="mt-8 space-y-4">
                <button
                  ...
                  className="w-full py-4 rounded-xl font-black text-lg uppercase text-black transition-all hover:scale-[1.02] disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center gap-3 font-headline cursor-pointer"
                  style={{
        ...
        {!carregandoInicial && passo === 'validando' && invocador && (
          ...
          >
            <div className="bg-black/60 backdrop-blur-xl rounded-3xl border border-white/5 p-6 shadow-2xl">
              <h3 className="text-white/20 text-[10px] font-black uppercase tracking-[0.2em] mb-4 font-headline">Invocador Selecionado</h3>
              <div className="flex items-center gap-4">
                <div className="relative">
                ...
                <div>
                  <p className="text-xl font-black text-white tracking-tight font-headline">{invocador.riotId}</p>
                  <p className="text-[10px] text-[#FFB700] font-bold uppercase tracking-widest mt-0.5 font-headline">✅ Conta Riot vinculada</p>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div className="relative text-center">
                ...
                <h2 className="text-2xl font-black text-white uppercase italic tracking-tight mb-2 font-headline">
                ...
                <button
                  onClick={handleVincularDiscord}
                  disabled={loadingDiscord}
                  className="w-full py-4 rounded-xl font-black text-lg uppercase text-white transition-all hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-3 mb-4 font-headline cursor-pointer"
        ...
        {!carregandoInicial && passo === 'verificando' && !contaVinculada && (
          ...
          >
            ...
            <div className="relative">
              ...
              <h2 className="text-3xl font-black text-white mb-4 tracking-tight uppercase italic font-headline">Sincronizando Dados</h2>
              ...
              <button 
                ...
                className="px-8 py-3 rounded-xl bg-white/5 border border-white/10 text-white/40 hover:text-white/60 text-xs font-black tracking-widest transition-all uppercase font-headline cursor-pointer"
              >
                Cancelar Processo
              </button>
            </div>
        ...
        {!carregandoInicial && passo === 'ja_vinculada' && jaVinculadaInfo && (
          ...
          >
            ...
            <div className="relative">
              ...
              <h2 className="text-3xl font-black text-white mb-4 tracking-tight uppercase italic leading-none font-headline">
              ...
              <div className="flex justify-center">
                <button
                  onClick={handleReiniciar}
                  className="px-12 py-4 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 font-black text-sm uppercase hover:bg-red-500/20 transition-all flex items-center justify-center gap-2 font-headline cursor-pointer"
                >
