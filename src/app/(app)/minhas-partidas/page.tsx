...
type ModoJogo = '1v1' | '5v5' | 'time_vs_time' | 'aram';

const MODO_INFO: Record<string, { icone: string; cor: string }> = {
  '1v1':          { icone: '⚔️', cor: '#FFB700' },
  '5v5':          { icone: '🎮', cor: '#00D4FF' },
  'time_vs_time': { icone: '🛡️', cor: '#FF6600' },
  'aram':         { icone: '🌀', cor: '#9146FF' },
};

const getModoInfo = (modo: string) => MODO_INFO[modo] || { icone: '🎮', cor: '#888' };
...
export default function MinhasPartidas() {
  ...
  const carregarDados = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/partidas?userId=${perfil.id}`);
      if (!res.ok) { setPartidas([]); setLoading(false); return; }
      const data = await res.json();
      const minhasPartidas: Partida[] = data.partidas || [];
      setPartidas(minhasPartidas);

      let vitorias = 0;
      minhasPartidas.forEach(partida => {
        const jogador = partida.jogadores.find((j: any) => j.id === perfil.id);
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="rounded-xl p-6 border border-white/10 animate-pulse" style={{ background: 'rgba(13,13,13,0.8)' }}>
        ...
        {loading ? (
          <div className="space-y-3">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="rounded-xl border border-white/10 p-5 animate-pulse flex gap-4 items-center" style={{ background: 'rgba(13,13,13,0.8)' }}>
                <div className="w-1 self-stretch rounded-full bg-white/10 shrink-0" />
          ...
          <div className="relative">
            <div className="space-y-3">
              {partidasExibidas.map((partida, index) => {
                const vitoria = perfil ? isVitoria(partida, perfil.id) : false;
                const jogador = perfil ? partida.jogadores.find((j) => j.id === perfil.id) : null;
                const modoInfo = getModoInfo(partida.modo);
                ...
                return (
                  <motion.div
                    ...
                    onClick={() => isClickable && router.push(`/sala/${partida.id}`)}
