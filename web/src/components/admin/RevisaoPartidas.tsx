...
function BotaoDecidir({
  ...
  return (
    <button
      ...
      className={`flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest border transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-sm ${cor}`}
    >
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-white font-black text-sm uppercase tracking-widest">Revisão de Partidas</h2>
        <button
          onClick={carregar}
          disabled={carregando}
      ...
      {/* Contestações de resultado */}
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <Gavel className="w-4 h-4 text-amber-400" />
          <h3 className="text-zinc-300 font-black text-sm uppercase tracking-widest">Contestações</h3>
          {!carregandoDisputas && disputas.length > 0 && (
            <span className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-amber-500/15 border border-amber-500/30 text-amber-400 text-[10px] font-black uppercase tracking-widest">
        ...
        {carregandoDisputas && disputas.length === 0 && (
          <div className="rounded-2xl p-8 flex items-center justify-center gap-3 shadow-xl" style={CardStyle()}>
            <Loader2 className="w-5 h-5 text-primary animate-spin" />
            <p className="text-zinc-400 text-sm font-bold">Carregando contestações...</p>
          </div>
        )}
        ...
        {!carregandoDisputas && disputas.length === 0 && (
          <div className="rounded-2xl p-8 text-center shadow-xl" style={CardStyle()}>
            ...
            <p className="text-zinc-400 text-sm font-bold">Nenhuma contestação pendente.</p>
          </div>
        )}
        ...
        {disputas.map((d) => {
          ...
          return (
            <div key={d.id} className="rounded-2xl p-5 lg:p-6 space-y-5 shadow-xl" style={CardStyle()}>
              {/* Cabeçalho da disputa */}
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-amber-500/15 border border-amber-500/30 flex items-center justify-center shrink-0 shadow-sm">
              ...
              <div className="rounded-xl p-4 bg-[#0e1320] border border-white/10">
                <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-1">Resultado verificado</p>
                <p className="text-white font-black text-sm">
                  {d.winnerSide ? (
                    ...
                    <span className="text-zinc-300">{d.resultado || '—'}</span>
                  )}
                </p>
              </div>
              ...
              <div className="rounded-xl p-4 bg-amber-500/10 border border-amber-500/25">
                <p className="text-[10px] font-black uppercase tracking-widest text-amber-400 mb-1">Motivo</p>
                <p className="text-amber-100 text-sm">{d.motivo}</p>
              </div>

              {/* Print da contestação */}
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-2">Print da contestação</p>
                {d.contestacaoUrl ? (
                  <button
                    onClick={() => setLightboxContestacao({ url: d.contestacaoUrl, nomeJogador: d.nomeJogador })}
                    className="rounded-xl overflow-hidden border border-white/15 bg-[#0e1320] text-left cursor-pointer group shadow-md"
                  ...
                  >
                    ...
                    />
                    <p className="px-2.5 py-1.5 text-[10px] font-black text-zinc-400">Clique para ampliar</p>
                  </button>
                ) : (
                  <div className="flex items-center gap-2 px-4 py-3 rounded-xl border border-white/10 bg-[#0e1320] text-zinc-500 text-xs">
              ...
              {/* Estorno bloqueado */}
              {semSaldo && (
                <div className="flex items-center gap-2 px-4 py-3 rounded-xl border border-red-500/40 bg-red-500/15 text-red-300 text-xs font-bold">
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-1">
                <button
                  ...
                  disabled={processando}
                  className="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest border transition-all disabled:opacity-30 disabled:cursor-not-allowed bg-white/5 border-white/15 text-zinc-300 hover:bg-white/10 hover:text-white shadow-sm"
                >
                ...
                <button
                  ...
                  disabled={processando}
                  className="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest border transition-all disabled:opacity-30 disabled:cursor-not-allowed bg-green-500/20 border-green-500/40 text-green-300 hover:bg-green-500/30 shadow-sm"
                >
      ...
      {carregando && salas.length === 0 && (
        <div className="rounded-2xl p-10 flex flex-col items-center gap-3 shadow-xl" style={CardStyle()}>
          ...
          <p className="text-zinc-400 text-sm font-bold">Carregando fila de revisão...</p>
        </div>
      )}
      ...
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-zinc-300 font-black text-sm uppercase tracking-widest">Partidas em análise</h3>
        {!carregando && salas.length > 0 && (
          <span className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-primary/15 border border-primary/30 text-primary text-[10px] font-black uppercase tracking-widest">
      ...
      {!carregando && !erro && salas.length === 0 && (
        <div className="rounded-2xl p-10 text-center shadow-xl" style={CardStyle()}>
          <Check className="w-6 h-6 text-green-400 mx-auto mb-2" />
          <p className="text-zinc-400 text-sm font-bold">Nenhuma partida aguardando revisão.</p>
        </div>
      )}
      ...
      <div className="space-y-4">
        {salas.map((sala) => {
          ...
          return (
            <div key={sala.id} className="rounded-2xl p-5 lg:p-6 space-y-5 shadow-xl" style={CardStyle()}>
              {/* Cabeçalho da sala */}
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#0e1320] border border-white/10 text-zinc-400 text-[10px] font-black uppercase tracking-widest">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl p-4 bg-blue-500/10 border border-blue-500/25">
                  <p className="text-[10px] font-black uppercase tracking-widest text-blue-300 mb-1">Time A</p>
                  <p className="text-white font-black text-sm truncate">{sala.timeANome || 'Time A'}</p>
                  {sala.timeATag && <p className="text-zinc-400 text-[10px]">[{sala.timeATag}]</p>}
                </div>
                <div className="rounded-xl p-4 bg-red-500/10 border border-red-500/25">
                  <p className="text-[10px] font-black uppercase tracking-widest text-red-300 mb-1">Time B</p>
                  <p className="text-white font-black text-sm truncate">{sala.timeBNome || 'Time B'}</p>
                  {sala.timeBTag && <p className="text-zinc-400 text-[10px]">[{sala.timeBTag}]</p>}
                </div>
              </div>
                  ...
                  {sala.jogadores.map((j) => (
                    <span
                      ...
                      className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-bold ${
                        j.side === 'blue' ? 'bg-blue-500/15 border-blue-500/30 text-blue-300' : 'bg-red-500/15 border-red-500/30 text-red-300'
                      }`}
                    >
                      {j.nome}
                      <span className="text-[9px] font-black uppercase opacity-75">{LADO_LABEL[j.side as Lado]}</span>
                    </span>
                  ))}
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {sala.prints.map((p) => (
                      <button key={p.id} onClick={() => setLightboxPrint({ id: p.id, nomeJogador: p.nomeJogador })}
                        ...
                        />
                        <p className="px-2.5 py-1.5 text-[10px] font-black text-zinc-400 truncate">{p.nomeJogador}</p>
                      </button>
                    ))}
                  </div>
                )}
                ...
                <div className="rounded-xl border border-amber-500/35 bg-amber-500/10 p-4 space-y-2">
                  ...
                  {sala.disputas.map((d) => (
                    <div key={d.id} className="flex items-start gap-2 text-sm">
                      ...
                      <span className="text-amber-100">{d.motivo}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-1">
                <BotaoDecidir
                  ...
                  cor="bg-blue-500/20 border-blue-500/40 text-blue-300 hover:bg-blue-500/30"
                  icone={Check}
                ...
                <BotaoDecidir
                  ...
                  cor="bg-red-500/20 border-red-500/40 text-red-300 hover:bg-red-500/30"
                  icone={Check}
                  processando={processando}
                  onClick={(l) => decidir(sala, l)}
                />
                <BotaoDecidir
                  lado="draw"
                  label="Empate"
                  cor="bg-[#0e1320] border-white/15 text-zinc-300 hover:bg-white/10 hover:text-white"
                  icone={Scale}
                  processando={processando}
                  onClick={(l) => decidir(sala, l)}
                />
                <BotaoDecidir
                  lado="cancel"
                  label="Cancelar"
                  cor="bg-red-500/10 border-red-500/25 text-red-400 hover:bg-red-500/20"
                  icone={X}
                  processando={processando}
                  onClick={(l) => decidir(sala, l)}
                />
