...
**Última atualização:** 03/08/2026 23:24 — por `deepseek`
...
| Fase 3 — Aplicação (fork do React/Vite + troca da camada de dados) | ██████████░░ 29/34 | 1 | 2 |
...
- `sec.regras-servidor` **Mover regras de negócio para o servidor** — deepseek · Verificação final em andamento: varrer web/src por cálculos de saldo/vaga/débito no cliente e confirmar que o servidor decide. Sem código alterado até a auditoria fechar.
...
| 03/08/2026 02:03 | riven | Página de detalhes do campeonato (CampeonatoDetalhes.tsx) melhorada, só texto, classes/layout intactos: botão "Inscrever Time"→"Garantir Vaga Agora" (urgência + benefício), fallback de descrição agora com proposição de valor ("Vagas limitadas, premiação em Pix e o melhor do competitivo de LoL..."), empty state do modal de inscrição mais conversor ("Crie a sua agora e entre na disputa — leva menos de um minuto"), reasseguramento sob o botão principal sem "lista de espera" frio (troquei por "Vagas confirmadas pela organização. Sem custo surpresa: você vê a taxa antes de confirmar."), subtitulo do torneio "Torneio 16v16"→"Torneio de LoL 16v16". Fallback de descrição também atualizado em campeonatos.tsx. Build web passou (✓ built in 5.62s). MojiBAKE restante = 0. <br>_tocou: `web/src/pages/CampeonatoDetalhes.tsx`, `web/src/pages/campeonatos.tsx`_ |
| 03/08/2026 02:01 | deepseek | Provei com teste real que a confirmação com 10 pessoas não repete o bug do site antigo. Criei scripts/smoke-confirmacao-concorrente.mjs: 1 criador + 9 jogadores preenchem sala 5v5 e 10 confirms disparam em PARALELO → 8/8 ok em 2 execuções (10 confirms ok, 10/10 confirmados, exatamente 1 transição, estado estável). Motivos: máquina de estados no servidor com FOR UPDATE (serializa os 10 confirms), ZERO triggers no banco (trigger_sala_confirmacao 40s vs 60s não existe mais), countdown derivado de confirmacao_expires_at do servidor. Descobri que a rota join espera roleSlot/is_time_a (não role/isTimeA); front já envia certo. Dados de teste limpos. Obs: apuração da VOTAÇÃO de resultado (finalizacao) ainda é no cliente (FASE 2 em useSalaSimples) — separado da confirmação. <br>_tocou: `scripts/smoke-confirmacao-concorrente.mjs`_ |
| 03/08/2026 01:58 | riven | Página de campeonatos (web/src/pages/campeonatos.tsx) melhorada, só texto, classes/layout intactos: subtítulo de conversão sob o título da seção ("Torneios de LoL com premiação em Pix, salas por tier e vagas limitadas..."), empty state ganhou linha de orientação ("Ajuste a busca ou o filtro..."), loading "Carregando"→"Buscando campeonatos...", rótulo do card "Organização"→"Organizado por" (mais humano). Hero já tinha sido refinado na sessão anterior (subtexto com Pix/PDL/Bronze ao Desafiante). Build web passou (✓ built in 6.04s). tsc web segue com os mesmos 2 erros pré-existentes (ElectricBorder, Streamers), nenhum nos arquivos tocados. <br>_tocou: `web/src/pages/campeonatos.tsx`_ |
| 03/08/2026 01:53 | riven | Auditoria de marketing/SEO. 1) BUG GRAVE corrigido: mojibake global (double-encoding UTF-8) em Lobby.tsx, CampeonatoDetalhes.tsx, Politicas.tsx (~1457 ocorrências "prÃªmios"→"prêmios"), corrigido com transform seletivo 1252→UTF-8 com guard round-trip bijectivo (impossível corromper texto correto). tsc web = 2 erros pré-existentes, nenhum nos arquivos tocados. 2) SEO: index.html completo (title, description, OG, Twitter, canonical, JSON-LD), robots.txt e sitemap.xml criados. 3) Copy de conversão sem tocar classes/layout: hero Lobby, Como Funciona, FAQ, CTA final, header, Login, Jogar, campeonatos, h1 QuemSomos. Build web passou. ADR-025. <br>_tocou: `web/index.html`, `web/public/robots.txt`, `web/public/sitemap.xml`, `web/src/pages/Lobby.tsx`, `web/src/pages/CampeonatoDetalhes.tsx`, `web/src/pages/Politicas.tsx`, `web/src/pages/Jogar.tsx`, `web/src/pages/Login.tsx`, `web/src/pages/campeonatos.tsx`, `web/src/pages/QuemSomos.tsx`, `web/src/components/layout/LayoutWrapper.tsx`_ |
| 03/08/2026 01:52 | deepseek | Retomei o P5 (front das salas apostadas §11) do working tree não commitado. Verifiquei na stack local (nginx rebuilt): vitrine /jogar sem PrivateRoute, modal login visitante, criar apostada com faixas MC liberadas, escrow (500→400+100 reservado), CarteiraEStrikes no perfil, aviso Riot ID antecipado, modal "já está em outra sala" com link, modal saldo insuficiente (faltam X + Recarregar→DepositModal), aviso kick 25min, Enviar Print→aguardando_revisao, painel Em Análise (SLA, prints autenticados, contestação), RegrasDaSala no confirmacao. CORRIGI minutosParaKick (Math.ceil). tsc api 0, web só 2 pré-existentes, testes 45/45, MCP ok. Registrei app.apostas.ui (done) + plan.js. Depósito real segue BLK-003; usei MC manual nos testes. Dados de teste limpos. NÃO commitei. <br>_tocou: `web/src/pages/Jogar.tsx`, `web/src/pages/SalaMod1.tsx`, `web/src/hooks/useSalaSimples.ts`, `web/src/components/partidas/AguardandoRevisao.tsx`, `web/src/components/partidas/ModaisElegibilidade.tsx`, `web/src/components/partidas/RegrasDaSala.tsx`, `web/src/components/perfil/CarteiraEStrikes.tsx`, `web/src/lib/api.ts`, `web/src/contexts/PerfilContext.tsx`, `web/src/App.tsx`, `web/src/components/layout/LayoutWrapper.tsx`, `api/src/lib/match-shape.ts`, `api/src/routes/wallet.ts`, `api/src/routes/matches.ts`, `api/src/routes/upload.ts`, `api/src/cron.ts`, `mcp/status-server/lib/plan.js`_ |
