...
**Última atualização:** 18/08/2026 00:34 — por `deepseek`
...
| 16/08/2026 16:21 | deepseek | Bug: todos os membros de time apareciam como TOP na TimePage. Causa raiz dupla: (1) transform-times.js copiava m.lane do Supabase (TOP/JG/MID/...) direto para role_slot, que espera vocabulário novo (top/jungle/mid/...); (2) slotToLegacy em api/src/routes/teams.ts não reconhecia o formato legado e caía no fallback 'TOP'. Correção: transform traduz lane→role_slot; API aceita os dois formatos. Dados normalizados na VPS via UPDATE (112 linhas), nenhum time com duplicidade nas vagas principais. Commit fb582bf, deploy na VPS (build app + recreate nginx pq ficou apontando pro IP antigo do app). Site 200, API devolve lanes corretas. <br>_tocou: `api/src/routes/teams.ts`, `scripts/migrate/transform-times.js`_ |
| 16/08/2026 04:37 | deepseek | Fila de tournament codes por modo (ADR-042): match_codes ganhou coluna mode (migration 0020). Os 2 códigos BR050c8-* (ARAM 1v1/x1) marcados mode='1v1'; os 6 BR04fa2-* (Summoner's Rift 5v5 Tournament Draft) ficam mode NULL (genéricos para 5v5/aram/time_vs_time). atribuirCodigoPartida em match-flow.ts filtra por modo (isNull para genérico). Testes: suíte completa 106/106 pass, incluindo novo teste de isolamento por modo em estados.test.ts; migration validada em Postgres limpo (PGlite) e aplicada na VPS (8 códigos, 2 com mode 1v1). App rebuildado na VPS, health 200, filtro confirmado no container. Validação do win condition 1v1 (first blood / 100 CS) já existia e está coberta por testes (verificar-partida.test.ts). Push e23941b, merge ff na VPS. <br>_tocou: `db/schema/matches.ts`, `db/migrations/0020_separar_fila_codigos_x1.sql`, `api/src/lib/match-flow.ts`, `api/test/estados.test.ts`, `api/package.json`_ |
