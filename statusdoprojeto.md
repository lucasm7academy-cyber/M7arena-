...
**Última atualização:** 05/08/2026 15:08 — por `deepseek`
...
| Fase 3 — Aplicação (fork do React/Vite + troca da camada de dados) | ███████████░ 34/38 | 1 | 2 |
...
| Aplicação (React + Vite) | ███████████░ 27/29 | 1 | 1 |
...
- `app.payments` **Gateway de pagamento MC (Mercado Pago PIX)** — deepseek · Nova tabela mc_packages (migration 0010), rotas /api/payments/packages, /api/payments/mc/order, /api/payments/webhook, /api/payments/:orderId/status, webhook idempotente credita user_wallets.mc + ledger deposit, DepositModal passa a consumir a API própria. Design aprovado (ADR-031).
...
- `[~]` **Gateway de pagamento MC (Mercado Pago PIX)** `app.payments`<br>  Nova tabela mc_packages (migration 0010), rotas /api/payments/packages, /api/payments/mc/order, /api/payments/webhook, /api/payments/:orderId/status, webhook idempotente credita user_wallets.mc + ledger deposit, DepositModal passa a consumir a API própria. Design aprovado (ADR-031).
