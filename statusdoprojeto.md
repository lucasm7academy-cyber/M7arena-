...
| Fase 3 — Aplicação (fork do React/Vite + troca da camada de dados) | ███████████░ 36/40 | 1 | 2 |
...
| Aplicação (React + Vite) | ███████████░ 29/31 | 1 | 1 |
...
| 06/08/2026 02:18 | deepseek | ADR-034: punições manuais (advertências + ban) no lugar de strikes/suspensão automáticos. Removi kick de ociosidade, strike de abandono e suspensão do cron (só fantasma+saneamento). user_strikes→user_advertencias (migration 0013: match_id nullable, criado_por; users ganham audit de ban). 3 advertências = ban automático, que só sai com unban manual. Rotas: POST/DELETE /admin/advertencias, POST /admin/usuarios/:id/ban|unban, GET /admin/usuarios?q=. Painel: aba Punições (busca+advertir/remover/banir/desbanir+histórico). Perfil: contador X/3 + status de ban. Testes 73/73 API, tsc limpo (web só erros pré-existentes). Commit f748b10 na branch feat/advertencias-ban. ATENÇÃO: commit 3062c68 do outro CLI entrou na minha branch com backend parcial quebrado; f748b10 corrige. Pendente: aplicar migration 0013 na VPS + deploy. <br>_tocou: `api/src/lib/elegibilidade.ts`, `api/src/lib/match-flow.ts`, `api/src/cron.ts`, `api/src/routes/admin.ts`, `api/src/routes/profiles.ts`, `api/src/routes/matches.ts`, `api/test/cron.test.ts`, `api/test/elegibilidade.test.ts`, `db/schema/apostas.ts`, `db/schema/identidade.ts`, `db/migrations/0013_advertencias_ban.sql`, `web/src/pages/AdminPunicoes.tsx`, `web/src/pages/Admin.tsx`, `web/src/pages/perfil.tsx`, `web/src/contexts/PerfilContext.tsx`, `web/src/hooks/useSalaSimples.ts`, `web/src/api/salamod1.ts`, `web/src/lib/api.ts`, `web/src/components/perfil/CarteiraEStrikes.tsx`, `web/src/components/partidas/ModaisElegibilidade.tsx`_ |
| 06/08/2026 01:53 | deepseek | Implementei o SAQUE de MC via PIX (spec/plano 2026-08-06-saque-mc-pix, componente novo app.saque). Regras aprovadas: 100MC=R$1, mín. R$20, sem taxa, MC debitado na solicitação (withdrawal_hold; paid consolida; rejected devolve withdrawal_refund), snapshot do PIX, admin paga fora e confirma.

BACKEND commitado pelo outro agente (3062c68): migration 0012, lib/withdrawals.ts, rotas, 6 testes PGlite, SDK.

FRONT escrito/verificado, NÃO commitado: DepositModal com abas Depósito(padrão)/Saque; admin aba Saques (FIFO, Marcar Pago/Rejeitar).

Verificação: tsx --test 73/73, tsc api exit 0, tsc web 2 pré-existentes, build ok.

PENDENTE (usuário decide): commit front + migration 0012 na VPS + deploy + teste ao vivo. CUIDADO: working tree tem trabalho uncommitado de outro agente (Punicoes, advertencias/ban) — não commitar com git add -A sem revisar. app.saque doing até deploy. <br>_tocou: `db/schema/economia.ts`, `db/migrations/0012_magical_sharon_ventura.sql`, `api/src/lib/withdrawals.ts`, `api/test/withdrawals.test.ts`, `api/src/routes/withdrawals.ts`, `api/src/index.ts`, `web/src/lib/api.ts`, `web/src/components/modals/deposit/DepositModal.tsx`, `web/src/components/modals/deposit/DepositTab.tsx`, `web/src/components/modals/deposit/SaqueTab.tsx`, `web/src/components/admin/SaquesPix.tsx`, `web/src/pages/Admin.tsx`, `mcp/status-server/lib/plan.js`, `docs/superpowers/specs/2026-08-06-saque-mc-pix-design.md`, `docs/superpowers/plans/2026-08-06-saque-mc-pix.md`_ |
| 06/08/2026 01:33 | deepseek | Painel admin em tela cheia: rota /admin movida para fora do Layout em web/src/App.tsx (sem header/sidebar do site); Admin.tsx reescrito com sidebar vertical própria (tópicos antes horizontais), botão "Voltar ao site" com ArrowLeft, header mobile com voltar + scroll. tsc/vite build ok. Commit 3062c68 (feat/advertencias-ban, base=origin/main) incluiu também WIP de outro terminal (saque PIX: migrations 0012/0013, user_advertencias ADR-033). Merge ff em main, push ok (3d9af47..3062c68). API tsc exit 0. BLOQUEIO: VPS ficou inacessível durante o deploy (ssh e https timeout). git pull rodou, mas migrations 0012/0013 NÃO aplicadas e rebuild app/nginx NÃO feito. Pendente quando a VPS voltar: aplicar 0012 e 0013, depois compose up --build app nginx. <br>_tocou: `web/src/App.tsx`, `web/src/pages/Admin.tsx`_ |
...
| 05/08/2026 02:50 | deepseek | Print de resultado: lightbox da imagem crescia pouco porque estava capada em max-w-[min(480px,90vw)] max-h-[80vh]. Aumentei para max-w-[min(1400px,94vw)] max-h-[92vh] nos 3 lightboxes (AguardandoRevisao.tsx, RevisaoPartidas.tsx, ResultadoPartida.tsx). Commit 7b670aa push + deploy VPS (git pull + compose up --build app nginx). Health 200, lobby 200, bundle contém '1400px'. Docker Desktop local não abriu no Windows, então não subi a stack local (localhost:3000) — usuário pediu deploy direto. Pendente: usuário quer tentar desinstalar/reinstalar o Docker Desktop (risco: perde volumes/imagens locais; não afeta VPS). <br>_tocou: `web/src/components/partidas/AguardandoRevisao.tsx`, `web/src/components/admin/RevisaoPartidas.tsx`, `web/src/components/partidas/ResultadoPartida.tsx`_ |
| 05/08/2026 02:45 | deepseek | Revalidou e fechou sec.upload com correções. Já protegia: sessão m7_session (upload.ts:261-274), magic bytes+MIME+5MB, sanitize anti-traversal, sem sobrescrita, match-prints p/ participante confirmado/revisor com leitura autenticada. Call sites reais: equipes.tsx:298, TimePage.tsx:133, createCampPage.tsx:135, SalaMod1.tsx:175, AguardandoRevisao.tsx:88. verify-swap storage OK 0. Gaps corrigidos: team-logos agora valida dono/capitão (path=teamId, pasta /uploads/team-logos/<team_id>/; front passou o id, 2 linhas); public-images restrito a admin/organizador; Nginx bloqueou /uploads/match-prints/ (403) nos dois nginx.conf. Teste novo upload-permissao.test.ts 4/4. Evidência: tsc api exit 0; tsx --test 56/57 (falha pré-existente test-realtime.mjs, exige WS+env); web tsc só 2 pré-existentes. Pendente: HTTP e2e não rodou (Docker daemon down) e validação ao vivo do nginx. Nota: evidência antiga citava 'prefixo do userId' que não existia — agora isolamento por time. <br>_tocou: `api/src/routes/upload.ts`, `api/test/upload-permissao.test.ts`, `infra/nginx.conf`, `infra/nginx.local.conf`, `web/src/pages/equipes.tsx`, `web/src/pages/TimePage.tsx`_ |
| 05/08/2026 02:42 | deepseek | Melhoria do painel admin (/admin) a pedido do usuário, que reportou duplicação deixada pelo agente Riven. Confirmado: os 6 cards de atalho da aba "Visão" (Saldos/Ranking/Notícias/Highlights/Cargos/Contatos) duplicavam exatamente as abas do menu superior. Corrigi em web/src/pages/Admin.tsx: removi os atalhos duplicados e substituí por widget operacional real — card "Revisão de Partidas" com contagem viva da fila (api.revisao.pendentes()), só para admin/proprietario, com atalho para a aba revisao. Typecheck web: Admin.tsx limpo (2 erros pré-existentes do fork em ElectricBorder.tsx e Streamers.tsx). Build vite ok (8.75s, bundle Admin gerado). Não commitei nem fiz deploy. <br>_tocou: `web/src/pages/Admin.tsx`_ |
| 04/08/2026 15:40 | deepseek | 3 correções deployadas (commits 29905a8, ea90cea). (1) 403 /admin: getAdminUser não aceitava role 'proprietario' — content.ts agora normaliza proprietario=admin, organizador=organizer. (2) Riot ID obrigatório em toda sala (decisão do dono): elegibilidade.ts moveu a checagem antes do retorno de casual; teste atualizado. (3) Nome/tag da vaga derivados de game_accounts.handle (nick + #tag, como o legado). Backfill: 131/145 contas tinham users.riot_id NULL (espelho só nas rotas novas) → UPDATE 131, agora 145/145. tsc ok, testes 51/51, health 200. <br>_tocou: `api/src/lib/content.ts`, `api/src/lib/elegibilidade.ts`, `api/src/lib/match-shape.ts`, `api/src/routes/matches.ts`, `api/test/elegibilidade.test.ts`_ |
