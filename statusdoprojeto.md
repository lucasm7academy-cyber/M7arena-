...
**Última atualização:** 30/08/2026 22:02 — por `deepseek`
...
| Fase 3 — Aplicação (fork do React/Vite + troca da camada de dados) | ██████████░░ 47/55 | 4 | 3 |
...
| Aplicação (React + Vite) | ██████████░░ 40/46 | 4 | 2 |
...
- `app.arquitetura.campeonatos` **Arquitetura do módulo campeonatos (features/ + classificação server-side)** — deepseek · Merge c2b2f20 deployado na VPS (dev.m7arena.pro). Tasks 6-12 concluídas e buildadas (HTTP 200, API health 200). Falta Task 13 (verificação visual 1:1) — aguardando usuário testar /campeonatos/:id e /criar-campeonato.
- `app.campeonato.verificacao-serie` **Verificação de série de campeonato via código Riot (MD3/MD5)** — deepseek · COMO FUNCIONA: cada jogo = 1 serie MD3/MD5 com UM code (POST gerar-codigo). Cron varre em_andamento, busca partidas Riot, valida por roster (team_members accepted), vencedor por pertinencia ao roster (nunca teamId), fecha em bestOf. Fora roster = jogada normal + irregular=true (selo). Sem PUUID = nao verifica. Resultado no servidor. FEITO: backend (motor, migration 0021, cron, endpoints, shape) 127 testes health 200. PENDENTE: front (Historico stats, botao gerar-codigo, badge irregular).
...
- `[~]` **Arquitetura do módulo campeonatos (features/ + classificação server-side)** `app.arquitetura.campeonatos`<br>  Merge c2b2f20 deployado na VPS (dev.m7arena.pro). Tasks 6-12 concluídas e buildadas (HTTP 200, API health 200). Falta Task 13 (verificação visual 1:1) — aguardando usuário testar /campeonatos/:id e /criar-campeonato.
- `[~]` **Verificação de série de campeonato via código Riot (MD3/MD5)** `app.campeonato.verificacao-serie`<br>  COMO FUNCIONA: cada jogo = 1 serie MD3/MD5 com UM code (POST gerar-codigo). Cron varre em_andamento, busca partidas Riot, valida por roster (team_members accepted), vencedor por pertinencia ao roster (nunca teamId), fecha em bestOf. Fora roster = jogada normal + irregular=true (selo). Sem PUUID = nao verifica. Resultado no servidor. FEITO: backend (motor, migration 0021, cron, endpoints, shape) 127 testes health 200. PENDENTE: front (Historico stats, botao gerar-codigo, badge irregular).<br>  _evidência:_ `npx tsx --test test/*.test.ts → 127 pass/0 fail; npx tsc --noEmit exit 0; migration 0021 via psql na VPS → CREATE TABLE + 13 ALTERs + 3 index OK; GET /api/health 200`
- `[x]` **Apostas individuais (self-bet) em partidas ranqueadas Solo/Flex** `app.aposta-individual`<br>  Anti-fraude + histórico. jogadorEmJogo (espectador) rejeita criar bilhete se jogador já está em ranqueada (ja_em_jogo_ranqueada); detecção já rejeita partida que começou antes da aposta (tol. 2min). GET /api/bets/history agrega apostas individuais + salas apostadas (delta via ledger). Componente HistoricoApostas no perfil (total ganho/perdido MC). Falta deploy.<br>  _evidência:_ `api tsc exit 0; bet.test 23/23 (novos jogadorEmJogo anti-fraude). web tsc exit 0; vite build '6.70s'.`<br>  _concluído 30/08/2026 22:02 por deepseek_
...
|---|---|---|
| 30/08/2026 22:02 | deepseek | Endureci o anti-fraude e criei o histórico unificado. Anti-fraude: função jogadorEmJogo (espectador) na criação do bilhete REJEITA apostar se o jogador já está em ranqueada (ja_em_jogo_ranqueada) — só aposta fora de jogo, antes de entrar na fila. A detecção continua rejeitando partida que começou antes da aposta (tol. 2min): aposta é sempre sobre o PRÓXIMO jogo, nunca sobre um rolando. Histórico: GET /api/bets/history junta apostas individuais (bet_tickets/bet_legs) + salas apostadas (matchPlayers+matches, delta via ledger match_prize/match_loss). Novo componente HistoricoApostas no perfil (total ganho/perdido MC + lista). Validação: api tsc 0, bet.test 23/23, web tsc 0, vite build 6.70s. PENDENTE: aplicar migration 0022 na VPS + deploy. <br>_tocou: `api/src/lib/live-bets.ts`, `api/src/routes/bets.ts`, `api/test/bets.test.ts`, `web/src/lib/api.ts`, `web/src/components/perfil/HistoricoApostas.tsx`, `web/src/pages/perfil.tsx`_ |
| 30/08/2026 21:46 | deepseek | Implementei a Aposta Individual (self-bet): o jogador aposta MC em si mesmo na própria próxima partida ranqueada Solo(420)/Flex(440). Plataforma = casa (odds fixas). Fluxo: escolhe fila+mercados+stake, reserva MC (escrow), cron detecta via espectador da Riot e trava em_jogo (matchId), liquida via match-v5 (vitória/derrota, kills, first blood) pagando stake×odd ou retendo. Anti-fraude 2min; timeout 20min cancela/devolve; fantasma 3h anula. Backend: schema bet_tickets/bet_legs (m.0022), lib/bets.ts, lib/live-bets.ts, routes/bets.ts, cron. Testes: bets 21/21. Front: modal ApostaIndividual+ItemMercado em /jogar abaixo de Partidas Finalizadas. PENDENTE: aplicar m.0022 na VPS (psql) e deploy. Sem chave Riot a detecção cai no fallback de timeout. <br>_tocou: `db/schema/bets.ts`, `db/schema/index.ts`, `db/migrations/0022_bet_individual.sql`, `api/src/lib/bets.ts`, `api/src/lib/live-bets.ts`, `api/src/routes/bets.ts`, `api/src/index.ts`, `api/src/cron.ts`, `api/test/bets.test.ts`, `web/src/lib/api.ts`, `web/src/components/partidas/ApostaIndividual.tsx`, `web/src/components/partidas/ItemMercado.tsx`, `web/src/pages/Jogar.tsx`_ |
| 30/08/2026 15:29 | deepseek | Salvo no status: ADR-048 (mecanismo), ADR-049 (regras) e componente app.campeonato.verificacao-serie atualizado com o 'como funciona'. Rodada: (1) Backend de verificação de série via código Riot deployado — motor serie-campeonato.ts (vencedor por jogada pelo roster, conta vitórias MD3/MD5, irregular se jogou fora), migration 0021, cron varre em_andamento, endpoints gerar-codigo/verificar, shape. 127/127 testes, tsc 0, health 200. (2) Refactor layout CampeonatoDetalhes: bloco único full-width (abas largura total); Info/Resp/inscrição/regulamento-mobile dentro da Visão Geral; depois ajustado — Classificação na coluna principal e Info→Resp→Inscrição na lateral (grid lg:grid-cols-3). web tsc 0, build 0. Commits: 840a758 (backend), b627642 (layout), 3226b25 (visão geral). Deploy VPS (app p/ backend, nginx p/ front). PENDENTE: front da verificação (aba Histórico stats, botão gerar-código, badge irregular) e validação visual do usuário. <br>_tocou: `api/src/lib/serie-campeonato.ts`, `api/src/cron.ts`, `api/src/lib/tournament-shape.ts`, `api/src/lib/verificar-partida.ts`, `api/src/routes/tournaments.ts`, `api/test/serie-campeonato.test.ts`, `db/schema/tournaments.ts`, `db/migrations/0021_campeonato_verificacao_serie.sql`, `web/src/pages/CampeonatoDetalhes.tsx`, `web/src/components/campeonatos/VisaoGeral.tsx`_ |
| 30/08/2026 15:00 | deepseek | Ajuste no layout da Visão Geral do campeonato: a Classificação voltou para a COLUNA PRINCIPAL (left, maior largura) e, na LATERAL (direita), empilhados: Informações → Responsável → Regulamento (mobile) → Inscrição. Na viewport pequena (mobile) tudo empilha (grid-cols-1). Tudo dentro da aba Visão Geral. Validação: web tsc exit 0, vite build exit 0. Commit 3226b25, push origin feat/redesign-modais, fast-forward main VPS, docker compose up -d --build nginx recriado, https://dev.m7arena.pro/ 200. Aguardando usuário conferir novo layout em /campeonato/:id. <br>_tocou: `web/src/components/campeonatos/VisaoGeral.tsx`_ |
| 30/08/2026 14:55 | deepseek | Refactor de layout do CampeonatoDetalhes: a página deixou de ter grid de 2 colunas (abas lg:col-span-2 + lateral sticky) e virou um BLOCO ÚNICO full-width. A barra de abas ocupa a largura total do body, e os cards Informações + Responsável + botão de inscrição + botão Regulamento (mobile) foram movidos para DENTRO da aba Visão Geral, que agora é o bloco que reúne classificação + informações + responsável + inscrição. Banner/hero e botão Regulamento do topo preservados (não mudaram). Limpei imports ociosos do CampeonatoDetalhes (Coins/CreditCard/Calendar/Diamond/Users/ShieldCheck/UserCheck/Trophy/AlertCircle movidos à VisaoGeral). Validação: web tsc exit 0, vite build exit 0. Commit b627642, push origin feat/redesign-modais, fast-forward main na VPS, docker compose up -d --build nginx recriado, https://dev.m7arena.pro/ 200 e /api/health 200. Aguardando usuário validar visualmente o novo layout no /campeonato/:id (criou que o bloco único esteja como esperado e nada quebrou). <br>_tocou: `web/src/pages/CampeonatoDetalhes.tsx`, `web/src/components/campeonatos/VisaoGeral.tsx`_ |
| 30/08/2026 14:38 | deepseek | Backend da verificação de série de campeonato via código Riot (ADR-047) implementado e deployado. Schema (migration 0021): colunas codigo_partida/best_of/irregular/resultado_riot/serie_iniciada_at + tabela tournament_series_games. Motor serie-campeonato.ts: decide vencedor por jogada pelo roster (titulares+reservas), conta vitórias até bestOf (MD3=2/MD5=3), marca irregular se jogou alguém fora, persiste e libera código. Cron varre jogos em_andamento. API: POST gerar-codigo (MD5 na final) e verificar; campos no shape legado. Validação: migration aplicada em banco limpo e na VPS via psql; 9 testes novos + suite = 127/127; tsc 0; app recriado; health 200. PENDENTE: fatia de FRONT (aba Histórico com stats, badge irregular, botão gerar-código para capitão+membros) e teste visual 1:1. <br>_tocou: `api/src/lib/serie-campeonato.ts`, `api/src/cron.ts`, `api/src/lib/tournament-shape.ts`, `api/src/lib/verificar-partida.ts`, `api/src/routes/tournaments.ts`, `api/test/serie-campeonato.test.ts`, `db/schema/tournaments.ts`, `db/migrations/0021_campeonato_verificacao_serie.sql`_ |
| 30/08/2026 13:15 | deepseek | Placar invertido corrigido (sala 72, 1v1 first blood). Causa: resumoRiot clusterizava kills/ouro pelo teamId da Riot (100→blue, 200→red), que em custom game não corresponde ao nosso lado (quem cria o lobby define o 100). Resultado: BLUE 1x0 quando RED venceu. Fix: resumoRiot agora recebe mapa puuid→lado e clusteriza pelo lado real; matches.ts monta esse mapa dos playersEnriched. Verificado no banco: red=teamId100/firstBlood/kills=1, blue=teamId200/kills=0; GET /api/matches/72 → blue.kills=0, red.kills=1, vencedor=B, first_blood. Front só usa placar.<side>.kills e sala.vencedor. 118 testes passam, tsc 0. Commit 037de28, app recriado na VPS, health 200. <br>_tocou: `api/src/lib/match-shape.ts`, `api/src/routes/matches.ts`_ |
| 30/08/2026 13:06 | deepseek | Sair da vaga no celular não funcionava porque o botão "Sair da Vaga" (VagaSlot.tsx) só aparecia no hover (group-hover:opacity-100), e touch não tem hover. Adicionado: detecção de touch via matchMedia('(hover: none)') + estado mostrarSair; no celular, o toque na vaga ocupada (própria) alterna um overlay FICAR/SAIR; no desktop o hover continua igual. Estado reseta ao trocar de vaga. Validação: npx tsc --noEmit exit 0, npx vite build exit 0. Commit b943dcb, push origin feat/redesign-modais, fast-forward main na VPS, docker compose up -d --build nginx recriado, site 200 em https://dev.m7arena.pro. Aguardando usuário testar no celular (toque na vaga → FICAR/SAIR). <br>_tocou: `web/src/components/partidas/VagaSlot.tsx`_ |
| 30/08/2026 12:51 | deepseek | Chat da sala agora abre fechado por padrão (useState(false) em vez de true). Antes ele montava aberto em todo reload/entrada na sala. Agora surge minimizado com a bolinha vermelha de não lidas visível (badge arredondado — fix do clipPath da rodada anterior já cobria o corte). Comentário do topo do arquivo atualizado. Build/verify: npx tsc --noEmit exit 0, npx vite build exit 0. Commit 73b2443, push origin feat/redesign-modais, fast-forward main na VPS, docker compose up -d --build nginx recriado, site 200 em https://dev.m7arena.pro. Aguardando usuário validar: entrar na sala e conferir chat minimizado com bolinha. <br>_tocou: `web/src/components/partidas/ChatDaSala.tsx`_ |
| 30/08/2026 12:47 | deepseek | Fix visual no chat da sala (ADR-040): a bolinha vermelha de não lidas aparecia cortada em cima à direita do botão minimizado do ChatDaSala. Causa: o badge ficava DENTRO do <button> que tem clipPath (CUT_FRAME), e o clip cortava a parte que vazava para fora. Correção: movi o badge para fora do button, como irmão, dentro de um wrapper relative inline-block, então ele flutua sem ser cortado. Design/alteração preservados. Build e verificação: npx vite build exit 0, npx tsc --noEmit exit 0. Commit a04c5e7, push origin feat/redesign-modais, fast-forward do main na VPS, docker compose up -d --build nginx recriado, site 200 em https://dev.m7arena.pro. Aguardando usuário validar visualmente no chat minimizado. |
| 28/08/2026 22:02 | deepseek | Corrigidos contraste de texto e largura do top bar da sala, deployado em dev.m7arena.pro. 1) players.tsx badge de elo: fundo virou bg-[#0a0a0a] escuro, mantendo borda e texto na cor do elo. 2) Jogar.tsx modal criar sala: card de modo selecionado passou de `cor25` para color-mix(cor 18%, #0d0d10) escuro, nome em white. 3) Jogar.tsx finalizadas: badge Vitoria Azul/Vermelho passou para texto #fff sobre color-mix(cor 22%, #0d0d10), borda cor80 mantida. 4) SalaMod1.tsx: top bar da sala ganhou w-full. Evidencias: tsc exit 0, vite build ok (6.22s), commit 8afb880, push vps main ff, nginx rebuild, site 200 em / /players /jogar, bundle com 0d0d10. Sem componentes do plano alterados (fix de UI). <br>_tocou: `web/src/pages/players.tsx`, `web/src/pages/Jogar.tsx`, `web/src/pages/SalaMod1.tsx`_ |
| 23/08/2026 23:54 | gemini | Corrigidas todas as bordas chanfradas (cut-edge) em cards, botoes, tags de times, banners e modais de navegacao e lobby, equipes, salas e perfil. Build gerada e deployada com sucesso na VPS (https://dev.m7arena.pro). <br>_tocou: `web/src/components/layout/LayoutWrapper.tsx`, `web/src/pages/Lobby.tsx`, `web/src/pages/equipes.tsx`, `web/src/pages/Jogar.tsx`, `web/src/pages/TimePage.tsx`, `web/src/pages/SalaMod1.tsx`, `web/src/pages/perfil.tsx`_ |
| 23/08/2026 23:33 | deepseek | Fix na VPS: página /players voltou a listar jogadores. Causa raiz: GET /api/players/filtrados exigia autenticação (401) enquanto a página é pública — o front engolia o erro com .catch(() => []) e mostrava lista vazia. No site antigo a RPC buscar_jogadores_filtrados era SECURITY DEFINER (diretório público). Tornados públicos: /api/players/filtrados e /api/wallet/admin/balances com userIds (agregação sem userIds continua só admin). Banco tinha 189 contas LoL; API agora devolve 191 sem login (HTTP 200). Build + deploy na VPS (m7arena_app e m7arena_realtime recriados). Verificado: filtrados sem auth 200, com auth 200, home 200. <br>_tocou: `api/src/routes/players.ts`, `api/src/routes/wallet.ts`_ |
| 22/08/2026 22:25 | deepseek | Fix deployado: aba "Chaves" do campeonato agora renderiza o bracket INLINE no body (handleTabClick deixou de interceptar 'bracket' para abrir o BracketModal em tela cheia; o modal continua acessível pelo botão "Ver em Tela Cheia" dentro do Chaves). Commit 51458dd mergeado em feat/redesign-modais e deployado na VPS (c2b2f20→51458dd), nginx recriado, site HTTP 200. Agendar verificação do usuário na aba Chaves. <br>_tocou: `web/src/features/campeonatos/CampeonatoContext.tsx`_ |
| 22/08/2026 20:52 | deepseek | Merge da branch arquitetura/campeonatos (c2b2f20) em feat/redesign-modais + deploy na VPS. Git merge main na VPS (911a58c→c2b2f20 fast-forward), docker compose rebuild app+realtime+nginx. Site: HTTP 200, API health: 200. Testes API: 118/118. Branch local: feat/redesign-modais c2b2f20. Resta Task 13 (verificação visual 1:1). <br>_tocou: `D:/Aplicativos/M7arenaSite/.git`, `dev.m7arena.pro`_ |
