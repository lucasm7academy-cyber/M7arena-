...
**Última atualização:** 04/08/2026 01:14 — por `deepseek`
...
| Fase 3 — Aplicação (fork do React/Vite + troca da camada de dados) | ██████████░░ 30/35 | 1 | 2 |
...
| Aplicação (React + Vite) | ███████████░ 26/28 | 1 | 1 |
...
- `app.ajustar-salas` **Força-tarefa: sync de timers e contagem de salas (plano ajustarsala)** — deepseek · Investigacao concluida e plano escrito em ajustarsala.md. Bug D (contas bloqueadas) causado por sala 2 presa em 'finalizacao' (estado morto do ADR-027) segurando linked=true; dados destravados na VPS (one-off). Bugs A/B/C analisados no codigo: contagem depende 100% do WS p/ quem ja esta na sala; timer usa Date.now() local (sem skew); 1v1 provavel efeito combinado do A. Proxima fase: validar plano com o usuario antes de codificar.
...
- `[~]` **Força-tarefa: sync de timers e contagem de salas (plano ajustarsala)** `app.ajustar-salas`<br>  Investigacao concluida e plano escrito em ajustarsala.md. Bug D (contas bloqueadas) causado por sala 2 presa em 'finalizacao' (estado morto do ADR-027) segurando linked=true; dados destravados na VPS (one-off). Bugs A/B/C analisados no codigo: contagem depende 100% do WS p/ quem ja esta na sala; timer usa Date.now() local (sem skew); 1v1 provavel efeito combinado do A. Proxima fase: validar plano com o usuario antes de codificar.<br>  _evidência:_ `Investigacao: ajustarsala.md escrito; rg finalizacao api/src = 0 ocorrencias (estado morto); SQL confirmou sala 2 finalizacao com 2 linked=true; UPDATE sala 2 -> encerrada + linked=false aplicado na VPS (contas destravadas).`
