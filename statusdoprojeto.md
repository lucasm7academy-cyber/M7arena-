...
**Última atualização:** 13/09/2026 23:18 — por `gemini`
...
| 11/09/2026 20:24 | gemini | Ajuste do card finalizado: transformado em barra de largura inteira (w-full) no topo do card com a palavra FINALIZADA em tamanho grande (fonte Anton) na cor primária e data/hora na direita; abaixo fica o confronto centralizado com o vencedor na esquerda e perdedor na direita, ambos com tamanhos generosos e proporções esportivas. Deploy na VPS concluído. <br>_tocou: `web/src/components/campeonatos/ListaCronograma.tsx`_ |
| 11/09/2026 20:20 | gemini | Ajuste visual do card finalizado: time vencedor posicionado na ESQUERDA (com placar verde) e time perdedor na DIREITA (com placar vermelho); o selo FINALIZADA deixou de ter corte diagonal e passou a ter cantos arredondados (rounded-full) com a cor primária do campeonato. Build e deploy no Nginx da VPS concluídos com sucesso. <br>_tocou: `web/src/components/campeonatos/ListaCronograma.tsx`_ |
| 11/09/2026 20:16 | gemini | Ajuste visual do card finalizado: o time perdedor sempre é exibido na esquerda e o vencedor na direita; cores originais preservadas nas bordas/tags dos times (verde apenas no número do placar do vencedor); banner FINALIZADA com a cor primária do campeonato; removidos badges MD3, string BRT e texto Súmula. Build validado e deploy na VPS concluído. <br>_tocou: `web/src/components/campeonatos/ListaCronograma.tsx`_ |
| 11/09/2026 20:07 | gemini | Design do card de partida finalizada no cronograma dos campeonatos: adicionado banner inicial cortado na diagonal com a palavra FINALIZADA na fonte Anton (padrão das salas), exibição de data/hora original da partida, destaque esportivo com badge VITÓRIA e glow para o vencedor, scoreboard central em caixa escura contrastada, e ação discreta de súmula para admins. Build verificado e deploy do container nginx concluído na VPS. <br>_tocou: `web/src/components/campeonatos/ListaCronograma.tsx`_ |
| 11/09/2026 19:33 | deepseek | Fase de grupos agora pontua por série (MD3/MD5), não por mapa. GroupStage.tsx e CampeonatoContext.getStatsForTeamForCalculation passaram a contar vitória de série = 1 V (adversário 1 D); J = séries jogadas. Alinha com o gerador de chaves (createCampPage, que já era por série). Classificação geral do servidor (buildClassificacao) ficou por mapa, a pedido do usuário. Validado com dados reais via script: MDR 2x1 RMD → MDR V=1/D=0; M7W 2x0 BKA → V=1/D=0 (antes somava mapas). Evidência: tsc web exit 0; deploy commit 0a0b6ae (rebuild nginx); bundle CampeonatoDetalhes-3AQdyYpJ→CnrurGZL; health 200. ADR-065. Falta hard-refresh do usuário na aba Grupos. <br>_tocou: `web/src/components/campeonatos/GroupStage.tsx`, `web/src/features/campeonatos/CampeonatoContext.tsx`_ |
| 11/09/2026 19:25 | deepseek | Bug do Lobby (#NKA em vez de #NKZA + logo sumida) corrigido. Causa: o time NKZ Academy foi renomeado NKA→NKZA, mas os snapshots tournament_matches.team_a_tag/team_b_tag continuavam 'NKA'; o Lobby usa a tag do snapshot e resolve logo por tag em times_inscritos, então a tag velha derrubava a logo. Fix dados: UPDATE por id ressincronizou tournament_matches e bracket_matches na VPS — ao vivo GET /api/tournaments/2bcef957 devolve timeA=NKZA e times_inscritos NKZA logo=true. Fix código: PUT /api/teams/:id propaga a tag nova aos snapshots (tournament_matches, bracket_matches) + array_replace em tournaments.seed_order; commit 77357a6 deployado (rebuild app). Evidência: tsc api exit 0, health 200, dist teams.js contém array_replace. ADR-064. Falta reload do usuário no Lobby. <br>_tocou: `api/src/routes/teams.ts`_ |
...
| 10/09/2026 02:33 | deepseek | Exclusão de inscrição e campeonato corrigidas (ADR-061, commit ea2516a, deployado). Causa 1: o X de excluir candidato só chamava o save legado (upsert) — a linha de tournament_teams nunca era apagada e o time voltava no reload. Causa 2: DELETE /tournaments/:id exigia ser o criador; o admin tomava 403 silencioso (LEITEIROS CUP tem organizador 57d73844, diferente do admin) e o campeonato voltava. Fix: rota DELETE /tournaments/:id/inscricoes/:teamId (organizador ou admin/proprietário) remove inscrição + classificação; DELETE do campeonato aceita admin/proprietário; front usa o endpoint dedicado e mostra falha no banner. Evidência: suíte 181/181, teste inscrição 2/2, rota sem auth → 401, bundle com removerInscricao no ar. Falta teste visual do usuário (remover M7K da COPA DO TESOURO). <br>_tocou: `api/src/lib/tournament-store.ts`, `api/src/routes/tournaments.ts`, `api/test/tournament-inscricao.test.ts`, `web/src/lib/api.ts`, `web/src/pages/createCampPage.tsx`, `mcp/status-server/lib/plan.js`_ |
