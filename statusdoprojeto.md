...
**Última atualização:** 18/08/2026 01:10 — por `gemini`
...
|---|---|---|
| 18/08/2026 01:10 | gemini | Redesign completo da pagina Lobby (Lobby.tsx) aplicando o padrao Cut-Edge, iluminacao de fundo e atmosfera escura. Criacao do arquivo de documentacao oficial DESIGNDOPROJETO.MD detalhando o design system, formulas de poligonos, gradientes e regras visuais. <br>_tocou: `web/src/pages/Lobby.tsx`, `DESIGNDOPROJETO.MD`_ |
| 18/08/2026 01:06 | gemini | Aplicação do estilo cut-edge (bordas chanfradas e contínuas) nos cards de modos de jogo, filtros de busca, cards de salas ativas/disponíveis e cards de partidas finalizadas (preservando o tom preto e branco/grayscale nas finalizadas, mas com o formato chanfrado). Build e deploy na VPS dev.m7arena.pro concluídos com sucesso (HTTP 200). <br>_tocou: `web/src/pages/Jogar.tsx`_ |
| 18/08/2026 01:03 | gemini | Remoção completa de animações pesadas e lentas de escala (grow-up, scale, spring bounce e delays) nos modais de elegibilidade (ModalShell), criação de sala (ModalCriarSala), depósito (DepositModal), VIP (VipModal), perfil (perfil.tsx) e sala de partida (SalaMod1.tsx), tornando a abertura e exibição de popups instantânea, ultrarresponsiva e leve. Build e deploy na VPS dev.m7arena.pro concluídos com sucesso (HTTP 200). <br>_tocou: `web/src/components/partidas/ModaisElegibilidade.tsx`, `web/src/pages/Jogar.tsx`, `web/src/components/modals/deposit/DepositModal.tsx`, `web/src/components/modals/vip/VipModal.tsx`, `web/src/pages/perfil.tsx`, `web/src/pages/SalaMod1.tsx`_ |
| 18/08/2026 00:59 | gemini | Refinamento completo da página de Perfil / Minha Conta (perfil.tsx e CarteiraEStrikes.tsx) removendo todos os cantos arredondados (rounded-2xl, rounded-xl, rounded-lg) dos subcards e elementos internos (estatísticas de elo, top campeões, carteira, advertências, modal PIX, equipe, email e redes sociais) e aplicando o padrão angular cut-edge oficial da arena. Build e deploy na VPS dev.m7arena.pro concluídos com sucesso (HTTP 200). <br>_tocou: `web/src/pages/perfil.tsx`, `web/src/components/perfil/CarteiraEStrikes.tsx`_ |
| 18/08/2026 00:56 | gemini | Refinamento completo de bordas contínuas em todos os cantos cortados (clip-path chamfered) dos modais (ModaisElegibilidade, ModalCriarSala, DepositModal, DepositTab, SaqueTab, VipModal), usando CutCard e pares harmonizados de polígonos (FRAME/INNER, BUTTON/INNER, BADGE/INNER) para contorno 100% nítido e ininterrupto em todas as diagonais. Build e deploy na VPS dev.m7arena.pro concluídos com sucesso (HTTP 200). <br>_tocou: `web/src/components/partidas/ModaisElegibilidade.tsx`, `web/src/pages/Jogar.tsx`, `web/src/components/modals/deposit/DepositModal.tsx`, `web/src/components/modals/deposit/DepositTab.tsx`, `web/src/components/modals/deposit/SaqueTab.tsx`, `web/src/components/modals/vip/VipModal.tsx`_ |
...
| 18/08/2026 00:34 | deepseek | Corrigiu verificação 1v1 ARAM. Bug 1: buscaIdsRiot filtrava só queue=3130 (SR); partida ARAM (queue 3200) nunca achada. Agora queue pelo mapa: 1v1→3200, demais→3130. Bug 2: vencedor era por teamId (100/200), que não mapeia blue/red da vaga em custom (quem cria lobby vira 100); na sala 59 Zeu King (blue) estava teamId 200. Agora vencedor por puuid com fallback teamId. Teste novo "lados invertidos", suíte 111/111, tsc exit 0. Commit 6340d43, deploy VPS. Cron já resolveu sala 59 certo: blue (Zeu King) por first_blood. ATENÇÃO: trabalho não commitado de outro agente no working tree (ModaisElegibilidade.tsx, Jogar.tsx, SalaMod1.tsx — refactor de modais) deixado intacto. <br>_tocou: `api/src/lib/verificar-partida.ts`, `api/test/verificar-partida.test.ts`_ |
| 18/08/2026 00:31 | gemini | Redesign completo e elevação visual dos modais e popups da plataforma (ModaisElegibilidade, ModalLoginVitrine, ModalVincularConta, ModalSenhaSala) para o padrão visual supremo da M7 Arena (vidro escuro #0a0a0c, iluminação radial âmbar/azul, neon top accent, bordas douradas #FFB700, ícones 3D com halo e CTAs de alta conversão). Evidência: npx tsc --noEmit (exit 0) e npm run build (exit 0). <br>_tocou: `web/src/components/partidas/ModaisElegibilidade.tsx`, `web/src/pages/Jogar.tsx`, `web/src/pages/SalaMod1.tsx`_ |
| 17/08/2026 23:56 | deepseek | Continuou e finalizou a feature "reconciliar handle Riot (cron 3 dias)". Review da Task 2 (timer no index.ts) aprovado — verbatim vs spec. Verificação: npx tsc --noEmit -p api/tsconfig.json → exit 0; npx tsx --test "api/test/*.test.ts" → 110/110. Commit 3d98fab na branch feat/reconciliar-handle-riot (rebaseada em 22b1191 = VPS main da época, para permitir ff-only), com 5 arquivos: index.ts (timer), lib/reconciliar-handles.ts, test/reconciliar-handles.test.ts, docs/superpowers plan+spec. Push origin+vps, merge --ff-only na VPS main, docker compose build app + up -d app. Live: /api/health 200; logs do app mostram runReconciliacaoHandles executando no boot (429 da dev key Riot — esperado, BLK-006; preserva handle velho sem crash). Sem migration. Nenhum filtro external_id no SELECT é ok: coluna é NOT NULL no schema. Ficou dirty no working tree: docs/project-state.json, docs/status-log.jsonl, statusdoprojeto.md (gerados pelo MCP, não fazem parte do feature). <br>_tocou: `api/src/index.ts`, `api/src/lib/reconciliar-handles.ts`, `api/test/reconciliar-handles.test.ts`, `docs/superpowers/plans/2026-08-17-reconciliar-handle-riot.md`, `docs/superpowers/specs/2026-08-17-reconciliar-handle-riot-design.md`_ |
| 17/08/2026 23:29 | deepseek | Feature: código de partida + verificar restritos a participante ou staff. Server: shapeSala redige codigo_partida=null para anônimos/não-participantes (detail + listagem); POST /matches/:id/verificar aceita staff (streamer/organizador/admin/proprietario) além do participante confirmado, via novo helper eStaffSala em lib/acesso-sala.ts. Front SalaMod1: botões Copiar Código e Verificar Partida só para jogadorAtual ou staff. Verificação: api tsc exit 0, web tsc exit 0, vite build ok, suíte 110/110. Commit 22b1191 (feat/sala-permissoes), push origin+vps, merge ff na VPS main, rebuild app+nginx+realtime. Live: health/home/lista 200; sala 57 (partida_iniciada, tem código real) → anônimo recebe codigo_partida null, participante (sessão temporária) recebe o código BR04fa2-*. ADR-043 registrada. ATENÇÃO: trabalho do outro agente (reconciliar-handle-riot: index.ts timer, lib/reconciliar-handles.ts, teste, specs) ficou NÃO commitado no working tree — não é meu, deixei intacto. <br>_tocou: `api/src/lib/acesso-sala.ts`, `api/src/routes/matches.ts`, `api/src/routes/matches-actions.ts`, `web/src/pages/SalaMod1.tsx`_ |
| 17/08/2026 02:17 | deepseek | Feature reconciliar handle Riot (cron 3 dias) em andamento via SDD na branch feat/reconciliar-handle-riot. Spec+plano em docs/superpowers/. Motivo: jogador renomeou no LoL, PUUID valida mas nome (game_accounts.handle) fica velho. Task1 DONE+review aprovado: api/src/lib/reconciliar-handles.ts (runReconciliacaoHandles DI, lote 3, atualiza handle+users.riot_id, falha preserva) + teste 4/4. Task2 IMPLEMENTADA: timer 3 dias no api/src/index.ts. Evidencia: tsc exit 0, suite 110/110, smoke tsx (docker local desligado). PROXIMO: review Task2 (diff .superpowers/sdd/review-task2-handles.diff; reviewer cancelado), depois COMMIT+VPS conforme pedido do usuario (commit branch, push origin+VPS, merge ff, rebuild app; sem migration). Ledger: .superpowers/sdd/progress.md. <br>_tocou: `api/src/lib/reconciliar-handles.ts`, `api/test/reconciliar-handles.test.ts`, `api/src/index.ts`_ |
| 17/08/2026 01:44 | deepseek | Task 1 do recurso "reconciliar handle Riot" concluída: criado api/src/lib/reconciliar-handles.ts (transcrição verbatim do brief) + api/test/reconciliar-handles.test.ts. TDD: RED (ERR_MODULE_NOT_FOUND) → GREEN (4/4). Typecheck exit 0. Suíte completa 110/110. Único desvio: brief usava before/after com PGlite compartilhado, mas o módulo varre todas as contas de lol e os counts vazavam entre testes; troquei para beforeEach/afterEach no teste (módulo e asserts permanecem 100% verbatim). Nada commitado (regra do projeto). Relatório em .superpowers/sdd/task-1-report.md. <br>_tocou: `api/src/lib/reconciliar-handles.ts`, `api/test/reconciliar-handles.test.ts`_ |
