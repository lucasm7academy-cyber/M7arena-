...
**Última atualização:** 07/09/2026 17:09 — por `gemini`
...
| 07/09/2026 17:02 | gemini | Análise e esclarecimento ao usuário sobre o funcionamento do fluxo de código de torneio e apuração automática de partidas no campeonato DEMOSTRATIVO (ADR-047/ADR-048/ADR-049), confirmando que o código já foi gerado e está em andamento, e explicando as regras de resolução via Riot API. |
| 07/09/2026 16:37 | gemini | Removido degradê das bordas pintadas do card hero do banner na página de detalhes do campeonato (web/src/pages/CampeonatoDetalhes.tsx), tornando a borda sólida na cor do tema do campeonato (campeonato.themeColor || '#FFB700') uniforme em todo o contorno sem fading/transparência. Build do Vite e typecheck validados com sucesso (exit 0). Commit 934a6d7 mergeado na branch main da VPS, container nginx rebuildado e novo bundle servido com HTTP 200 em dev.m7arena.pro. <br>_tocou: `web/src/pages/CampeonatoDetalhes.tsx`_ |
| 06/09/2026 04:50 | gemini | Validação completa do ciclo de séries de campeonato e verificação na Riot: 1) Botão Iniciar Série no cronograma: exibe 5 min antes do horário (ou horário alcançado/passado) para participantes do confronto e staff. 2) Cópia do Código Riot de Torneio restrita aos membros dos 2 times e staff; espectadores veem 'Em Andamento' e placar da série ao vivo. 3) Motor de resolução (MD3/MD5): validação por PUUID dos rosters cadastrados, tolerância com flag 'irregular: true' para jogadores fora do roster, atualização em tempo real de scoreA/scoreB e fechamento automático ao atingir o limite de vitórias. 4) Varredura periódica via cron (10min) + acionamento instantâneo via botão 'Verificar'. 5) Testes (10/10 serie-campeonato, 9/9 cron/classificacao) passando, tsc exit 0 em api e web, build do Vite exit 0, healthcheck de produção na VPS retornando 200 OK. Componente app.campeonato.verificacao-serie concluído. <br>_tocou: `api/src/routes/tournaments.ts`, `api/src/lib/serie-campeonato.ts`, `api/src/lib/tournament-shape.ts`, `api/src/cron.ts`, `api/test/serie-campeonato.test.ts`, `web/src/components/campeonatos/ListaCronograma.tsx`, `web/src/features/campeonatos/CampeonatoContext.tsx`, `web/src/lib/api.ts`, `app.campeonato.verificacao-serie`_ |
| 06/09/2026 03:38 | gemini | Atualização do fluxo de Regulamento dos campeonatos: 1) Em FormCriarCampeonato.tsx e no modal de edição de createCampPage.tsx, o campo de regulamento foi transformado em campo de URL/Link oficial (Google Docs/PDF/página externa). 2) Em createCampPage.tsx, corrigida a persistência do regulamento no handleSaveEdit que não gravava ao salvar edição. 3) Em CampeonatoDetalhes.tsx e VisaoGeral.tsx, o clique no botão/card de Regulamento agora abre diretamente uma nova aba (_blank) com o link informado, com fallback para o modal de regras caso haja texto legado. 4) No RegrasModal.tsx, adicionado botão de abrir documento oficial em nova aba. 5) Verificado com tsc (exit 0), vite build (exit 0), commit f14044e, push origin e deploy na VPS com container nginx rebuildado (HTTP 200 ao vivo). <br>_tocou: `web/src/pages/CampeonatoDetalhes.tsx`, `web/src/features/campeonatos/components/FormCriarCampeonato.tsx`, `web/src/pages/createCampPage.tsx`, `web/src/features/campeonatos/components/modals/RegrasModal.tsx`_ |
| 06/09/2026 03:12 | gemini | Configuração flexível de restrição de campeonatos por pontos ou Free Elo: 1) Em FormCriarCampeonato e no modal de edição de createCampPage, adicionado seletor com presets rápidos de 1 clique ('Free Elo', '300 Pontos', '500 Pontos', '700 Pontos', '1000 Pontos', 'Tier I', 'Tier II') e campo de texto livre para quantidade personalizada de pontos. 2) Em campeonatos.tsx e VisaoGeral.tsx, o rótulo foi tornado dinâmico para exibir 'Limite: X Pontos' / 'Limite de Pontos: X Pontos' quando configurado por pontos, mantendo 'Tier: Free Elo' / 'Tier I'. 3) Verificado com tsc --noEmit (exit 0), vite build (exit 0), commit c8b93cb e deploy na VPS com rebuild do container nginx (HTTP 200 ao vivo em m7arena.pro). <br>_tocou: `web/src/features/campeonatos/components/FormCriarCampeonato.tsx`, `web/src/pages/createCampPage.tsx`, `web/src/pages/campeonatos.tsx`, `web/src/components/campeonatos/VisaoGeral.tsx`_ |
| 05/09/2026 22:14 | gemini | Correção do redirecionamento para dev.m7arena.pro/login no fluxo de autenticação e campeonatos. 1) No backend (api/src/routes/auth-google.ts), appUrl e origemHost foram ajustados para priorizar o host da requisição (m7arena.pro) e ter fallback para https://m7arena.pro, além de capturar mensagens de erro do Google e logar warnings. 2) No .env da VPS e docker-compose.yml, APP_URL e MERCADO_PAGO_NOTIFICATION_URL foram atualizados de dev.m7arena.pro para m7arena.pro. 3) CORS_ALLOWLIST e allowedOrigins no realtime mantiveram dev.m7arena.pro apenas como fallback. 4) No front (web/src/pages/Login.tsx), adicionado useEffect para capturar ?erro= da URL e exibir alerta traduzido em português. Validação: tsc api/web exit 0, vite build ok, teste curl https://m7arena.pro/api/auth/google/callback redirecionando para https://m7arena.pro/login?erro=state_invalido (testado ao vivo). ADR-054 registrada. <br>_tocou: `api/src/routes/auth-google.ts`, `api/src/index.ts`, `api/src/realtime/index.ts`, `web/src/pages/Login.tsx`, `infra/docker-compose.yml`, `VPS: /root/m7arena/.env`_ |
...
| 02/09/2026 23:02 | deepseek | Notificação+modal de resultado da aposta individual implementados e deployados (34de438 na main VPS). Backend: lib notifications.ts, rota /api/notifications (GET listar+unreadCount, POST /:id/read, POST /read-all, auth getAuthUser), cron de liquidacao/cancelamento/anulacao grava na tabela notifications. Front: Toaster global (destrava toasts invisiveis), ModalResultadoAposta (desfecho ganhou/perdeu/anulada), polling 20s so com aposta ativa, NotificationBell com badge+lida+som WebAudio, SDK api.notifications.*+betDeltaMc(). Verificado: web tsc+build ok, api tsc 0, bets.test 26/26. Live: health 200, site 200, notifications(anon) 401. Commitei so meus arquivos; worktree de outro agente intacto. PENDENTE: historico no perfil em branco apos desafio ganho — provavel bilhete nao liquidado (chave Riot dev BLK-006) impedindo deteccao; investigar com dado real. <br>_tocou: `api/src/lib/notifications.ts`, `api/src/routes/notifications.ts`, `api/src/lib/live-bets.ts`, `api/src/index.ts`, `web/src/App.tsx`, `web/src/lib/api.ts`, `web/src/lib/notification-sound.ts`, `web/src/components/partidas/ModalResultadoAposta.tsx`, `web/src/pages/ApostaIndividualPage.tsx`, `web/src/components/notifications/NotificationBell.tsx`_ |
| 02/09/2026 22:58 | deepseek | Deploy do sistema de elo (commit 69a8c15) na VPS. Fluxo: a VPS (/root/m7arena) tem 'main' checked out e o remote 'vps' aponta p/ ela — push direto na main é recusado (denyCurrentBranch=refuse). Solução: `git push vps <commit>:refs/heads/_deploy` e `ssh pandapost-vps "cd /root/m7arena && git merge --ff-only _deploy && git branch -d _deploy"`. ATENÇÃO: working tree local tem trabalho de OUTRO agente misturado (notificações + aposta individual em index.ts e lib/api.ts) — committo seletivo via git apply --cached (só meus hunks). Rebuild: `docker compose --env-file /root/m7arena/.env -f infra/docker-compose.yml up -d --build app nginx` (realtime recriado junto, compartilha imagem; não toquei no código dele). Verificado: health 200, site 200, refresh-elos anônimo=401. Chave Riot válida (job de handles toma 429 rate-limit no boot, pré-existente — não é do elo). <br>_tocou: `api/src/lib/atualizar-elos.ts`, `api/src/routes/players.ts`, `api/src/index.ts`, `web/src/pages/players.tsx`, `web/src/components/players/PlayerDetailModal.tsx`, `web/src/pages/Admin.tsx`, `web/src/lib/api.ts`_ |
